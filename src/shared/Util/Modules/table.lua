...
local function Count(tbl: {}): number
    local count = 0
    for _, _ in tbl do
        count += 1
    end
...
--[=[
    ...
    Recursively sets all of the keys in the table to nil, returns nil.
]=]
local function Clear(tbl: {}): nil
...
--[=[
    ...
    Gets the wrapped index in a table, and the previous index if 'prev' is true.
]=]
local function GetWrappedIndex(tbl: {}, index: number, prev: boolean?): (number, number?)
    local length = #tbl
    index = (index % length) + 1
    if prev then
        -- -2 since lua is 1-based indexing
        local prevIndex = ((index - 2) % length) + 1
...
--[=[
    ...
    Just uses table.sort, but returns the table for convenience.
]=]
local function Sort(tbl: {}, sorter: (a: any, b: any)->(boolean)): {}
...
--[=[
    ...
    Calls a function on every element in the table.
]=]
local function Apply(tbl: {}, callback: (value: any, key: any)->())
    for key, value in tbl do
        callback(value, key)
    end
end

local function ApplyOnChildren(parent: Instance, callback: (value: any, key: any)->(), deep: boolean?)
    Apply(deep and parent:GetDescendants() or parent:GetChildren(), callback)
end

TableUtil.ApplyOnChildren = ApplyOnChildren
TableUtil.Apply = Apply
TableUtil.Sort = Sort
TableUtil.GetWrappedIndex = GetWrappedIndex
